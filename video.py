...
class Download():
    ...
    def write(self, info):
            ...
            self.t.see(END)	# 始终显示最后一行，不加这句，当文本溢出控件最后一行时，不会自动显示最后一行
    ...
    def __init__(self, width=700, height=330):
        ...
        # set frame_1
        label1 = Label(frame_1, text='输入视频链接：')
        entry_url = Entry(frame_1, textvariable=self.url, width=35)

        # set frame_3
        label2 = Label(frame_2, text='视频输出地址：')
        entry_path = Entry(frame_2, textvariable=self.path, width=35)
        
        # set frame_2
        path = StringVar()
        url_path = Button(frame_3, text = "路径选择",  width=3,command = self.selectPath)
        down = Button(frame_3, text='下载', width=3, height=2,
                         command=self.video_download)
        
        label_desc = Label(frame_4, 
                              text='本项目已在GitHub上开源,遵守GNU通用公共许可证(GPL)')
        label_jnxxhzz = Label(frame_4,
...
#        label_img.pack(ipadx=100,ipady=0)
        ...
        self.t = Text(self.root)	# 创建多行文本控件
        self.t.pack()	# 布局在窗体上
