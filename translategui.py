...
class Translate():
    ...
    def __init__(self):
        ...
        self.input = ttk.Entry(self.window, width=80)  # 创建一个输入框,并设置尺寸
        self.info = tk.Text(self.window, height=18)   # 创建一个文本展示框，并设置尺寸
        # 添加一个按钮，用于触发翻译功能
        self.t_button = ttk.Button(self.window, text='翻译',  command=self.fanyi)
        # 添加一个按钮，用于触发清空输入框功能
        self.c_button1 = ttk.Button(self.window, text='清空输入', command=self.cle_e)
        # 添加一个按钮，用于触发清空输出框功能
        self.c_button2 = ttk.Button(self.window, text='清空输出', command=self.cle)
        # 添加一个按钮，用于打开GitHub仓库
        self.c_button3 = ttk.Button(self.window, text='本代码开源，点击前往GitHub了解更多', command=self.GoToGitHub)
        # 添加一个按钮用于将内容复制到剪贴板
        self.c_button5 = ttk.Button(self.window, text='将结果复制到剪贴板', command=self.copyit)
        self.input.bind("<Return>", self.gotomaain)
        # 添加一张图标
        # self.image_file = ttk.PhotoImage(file='py128.png')
        # self.label_image = ttk.Label(self.window, image=self.image_file)
