...
def goupi_main():
    ...
    RepeatLevel.set(2)
    ttk.Label(m,text='欢迎使用狗屁不通文章生成器！\n请输入狗屁不通文章主题：').pack()
    ttk.Entry(m,textvariable=maintheme).pack()
    ttk.Label(m,text='请输入重复度，建议为2').pack()
    ttk.Entry(m,textvariable=RepeatLevel).pack()
    ttk.Button(m,text='开始输出',command=lambda:start_main(xx=maintheme.get(),rep=RepeatLevel.get())).pack()
    ttk.Button(m,text='复制到剪贴板',command=lambda:pyperclip.copy(resultit)).pack()
    global putoutgp
    putoutgp=tk.Text(m,font=('等线',9))
    putoutgp.pack()
    ttk.Label(m,text='纯属娱乐！！！本人不对输出内容负责，请勿违反道德和法律，\n制作者不对输出内容负任何责任！！').pack()
    m.mainloop()
