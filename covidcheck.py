...
def createcovidtoplevel():
    ...
    covid.title="疫情查询和下载"
    ttk.Label(covid,text="请输入你要查询的省份(要加“省市自治区”字样)").pack()
    a=ttk.Entry(covid,textvariable=kkk)
    a.pack()
    ttk.Button(covid,text="查询",command=lambda:Parse_data2(kkk.get())).pack()
    ttk.Button(covid,text="全国疫情形势(数据+地图)",command=lambda:display_provinces()).pack()
    ttk.Label(covid,text="众志成城，共克时艰！").pack()
    ttk.Label(covid,text="数据来源：国内数据来自中华人民共和国国家卫生与健康委员会\n由于卫健委api格式在不停变化，该功能可能会失效\n绘制出的可视化地图非官方制图，请勿用于商业用途\n软件及绘制出的图像由BillMa007版权所有").pack()
    covid.mainloop()
